...
@pytest.mark.asyncio
async def test_websocket_spectrogram():
...
@pytest.mark.asyncio
async def test_stream_endpoint_ffmpeg_mock():
    ...
    with patch("asyncio.create_subprocess_exec", return_value=mock_proc) as mock_exec:
        with patch.object(processor, "subscribe_audio", return_value=mock_audio_queue):
             with patch("src.live.server.feed_input", new_callable=AsyncMock) as mock_feed: # Prevent actual feeding loop
                
                response = client.get("/stream?source=test_mic", stream=True)
                assert response.status_code == 200
                
                # Consume stream
                chunks = list(response.iter_content())
                assert b"mp3_frame_1" in chunks
                assert b"mp3_frame_2" in chunks
