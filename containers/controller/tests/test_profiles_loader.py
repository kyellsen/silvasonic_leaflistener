...
from silvasonic_controller.profiles_loader import load_profiles, find_matching_profile, MicrophoneProfile, DetectedDevice, get_alsa_devices
import subprocess # for exceptions
...
def test_profile_loading(mock_profiles_dir):
...
def test_find_matching_profile_mock():
...
def test_find_matching_profile_auto_match():
    profiles = [
    ...
    with patch("silvasonic_controller.profiles_loader.get_alsa_devices", return_value=[mock_device]):
        profile, device = find_matching_profile(profiles)
        assert profile.name == "Rode"
        assert device == mock_device

def test_find_matching_profile_fallback():
    profiles = [
...
def test_find_matching_profile_forced():
...
def test_find_matching_profile_no_devices():
...
def test_real_scan_error():
    with patch("subprocess.run", side_effect=Exception("Boom")):
        devices = get_alsa_devices()
        assert devices == []

def test_load_profiles_not_exist(tmp_path):
    assert load_profiles(tmp_path / "nowhere") == []

def test_load_profiles_corrupt(mock_profiles_dir):
...
def test_find_profile_forced_no_devices():
...
def test_generic_fallback_fail():
