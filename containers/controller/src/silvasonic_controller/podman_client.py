...
class PodmanOrchestrator:
    ...
    def list_active_recorders(self) -> list[dict[typing.Any, typing.Any]]:
        ...
        try:
            ...
            if res.returncode == 0 and res.stdout.strip():
                ...
                try:
                    return typing.cast(list[dict[typing.Any, typing.Any]], json.loads(res.stdout))
    ...
    def spawn_recorder(self, name: str, profile_slug: str, device_path: str, card_id: str) -> bool:
        ...
        env_vars = [
            ...
            "-e",
            "LIVE_STREAM_TARGET=silvasonic_livesound",  # DNS name in compose network
            "-e",
            "SILVASONIC_DATA_DIR=/mnt/data/services/silvasonic",  # Internal path logic?
        ]
        ...
        volumes = [
            "-v",
            f"/dev/snd/pcmC{card_id}D0c:/dev/snd/pcmC{card_id}D0c",  # Specific device?
            # Actually, just map the specific device needed
            "--device",
            f"/dev/snd/controlC{card_id}",  # Control device needed for ALSA lib
            "--device",
        ...
        cmd = [
            ...
            # Networking
            "--network",
            "silvasonic_default",  # Assuming compose project name 'silvasonic'
            *env_vars,
            *volumes,
    ...
    def stop_recorder(self, container_id: str) -> None:
