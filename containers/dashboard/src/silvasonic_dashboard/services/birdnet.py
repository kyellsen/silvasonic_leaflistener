...
class BirdNetService:
    ...
    @staticmethod
    async def get_recent_processed_files(limit: int = 50) -> list[dict[str, typing.Any]]:
        """Get list of recently processed files (raw recordings)."""
        try:
            async with db.get_connection() as conn:
                query = text(
                    """
                ...
                for row in result:
                    ...
                    if d.get("file_size_bytes"):
                        d["size_fmt"] = BirdNetService._format_size(d["file_size_bytes"])
                    else:
                        d["size_fmt"] = "-"

                    # Format duration
                    if d.get("duration_sec"):
                        d["duration_fmt"] = BirdNetService._format_duration(d["duration_sec"])
                    else:
                        d["duration_fmt"] = "-"
    ...
    @staticmethod
    async def get_processing_stats() -> dict[str, typing.Any]:
        """Get aggregate stats for processed files."""
        try:
            async with db.get_connection() as conn:
                query = text(
                    """
                ...
                if res:
                    d = dict(res._mapping)
                    d["total_duration_fmt"] = BirdNetService._format_duration(
                        d.get("total_duration", 0)
                    )
                    d["avg_duration_fmt"] = BirdNetService._format_duration(
                        d.get("avg_duration", 0)
                    )
                    d["avg_size_fmt"] = BirdNetService._format_size(d.get("avg_size") or 0)
                    return d
                return {}
        except Exception as e:
