...
def test_validation():
    ...
    response = client.post(
        "/settings",
        data={"latitude": "91.0", "longitude": "13.40", "notifier_email": "test@example.com"},
    )
    assert response.status_code == 200
    ...
    assert (
        "ensure this value is less than or equal to 90" in response.text
        or "less than or equal to 90" in response.text
        or "1 validation error" in response.text
        or "Validation Error" in response.text
    )
